...
const TableRow = props => {
  ...
  return (
    <Table.Row>
      ...
      <Table.Cell
        className={`${props.event.probability >= 0.9 ? 'error' : ''} ${
          props.event.probability > 0.7 && props.event.probability < 0.9
            ? 'warning'
            : ''
        }`}
      >
      ...
      <Table.Cell>
        <button
          className={`circular ui toggle compact icon ${
            props.event.saved ? 'orange' : ''
          } button`}
        >
          <i className="check icon" onClick={_handleToggleSaved} />
        </button>
      </Table.Cell>
    </Table.Row>
  );
